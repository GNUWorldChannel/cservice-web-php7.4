...
<link rel='stylesheet' href='/css/font-awesome.min.css'>
	<title>PAGE LOCKED BY CSERVICE ADMINISTRATORS</title>";
	std_theme_styles();
	echo "</head>\n";
	std_theme_body();
	echo "<center>\n";
	echo "<div class='p-3 mb-2 bg-danger text-white'><h2>";
...
if ($curr_step==8) {
	if ($_POST["rCRC"]==md5( $_POST["username"] . CRC_SALT_0011 . $_SERVER["REMOTE_ADDR"] . $_POST["email"] . $_POST["gfxcode_val"] . CRC_SALT_0007 )) {
		...
		if (UNETUB_TIME>0) {
			...
			$cookie = md5( $expire . "Ratonera User Block");
...
switch ((int)$curr_step) {
	...
	case 6:
		...
		if ($err!="") { err_newuser($err); } else {
			...
			require_once('../../php_includes/recaptcha.php');
			echo "<div class='g-recaptcha' data-sitekey='6LfbhNwSAAAAAAQpCJxezyyOfcHRaCgvbH48df1p'></div>";
			//$publickey = "";
...
//			echo recaptcha_get_html($publickey);
	   ...
	   $secret = "6LfbhNwSAAAAAMjaIA01g__UXHBf0Q5q0h1nOXqE";
